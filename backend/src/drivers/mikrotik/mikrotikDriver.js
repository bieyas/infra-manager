 ...
 *
 * Supported capabilities: interfaces, ipAddresses, routes, neighbors,
 * pppoeSessions, resource, exec.
 */
export class MikrotikDriver extends BaseDriver {
  ...
  async connect() {
    ...
    try {
      ...
      throw this._connectionError(error, port)
    }
  }
  ...
  _connectionError(error, port) {
    const detail = error?.message || error?.toString?.() || 'unknown error'
    return new Error(`RouterOS API connection failed to ${this.device.ip}:${port}: ${detail}`)
  }
  ...
  async getResource() {
    ...
    if (!r) throw new Error('RouterOS API returned no resource data')
    return {
  ...
  async getPppoeSessions() {
    const list = await this._cmd('/ppp/active/print')
  ...
  async getPppSecrets() {
    const list = await this._cmd('/ppp/secret/print')
