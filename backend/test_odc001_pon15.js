// Test script untuk ODC-001 cascade update ke OLT-AUFA PON-15
import { PrismaClient } from '@prisma/client';

const prisma = new PrismaClient();

async function testOdc001Pon15() {
  console.log('🧪 Testing ODC-001 cascade update to OLT-AUFA PON-15...');
  ...
  try {
    ...
    console.log(`\n🎯 Target: OLT=${oltAuha.name} (${oltAuha.id}), PON=PON-15`);
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.cascadeChangeLog.create({
        data: {
          ...
          reason: 'Manual update ODC-001 to OLT-AUFA PON-15'
        }
      });
    ...
    const cascadeLogs = await prisma.cascadeChangeLog.findMany({
      where: {
        ...
        reason: { contains: 'PON-15' }
      },
      orderBy: { createdAt: 'desc' },
      take: 5
    });
    
    console.log('\n📋 Recent Cascade Logs (PON-15):');
    ...
    // Verification
    const odc001Correct = updatedOdc001.oltId === oltAuha.id && updatedOdc001.ponPort === 'PON-15';
    const odc002Correct = updatedOdc002.oltId === oltAuha.id && updatedOdc002.ponPort === 'PON-15';
    const odc003Correct = updatedOdc003.oltId === oltAuha.id && updatedOdc003.ponPort === 'PON-15';
    ...
    if (odc001Correct && odc002Correct && odc003Correct) {
      console.log('\n🎉 CASCADE UPDATE TO PON-15 PASSED!');
      console.log('✅ ODC-001 berhasil diubah ke OLT-AUFA PON-15');
